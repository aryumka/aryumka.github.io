...
쉽게 말해 Javascript의 `DOMTree`와 비슷하다고 보면 됩니다.~~근데 이제 절망버젼인 DOM~~
...
여기에 임의로 ~~억지로~~ 텍스트를 입력할 수 있도록(Fillable) 해주는 컴포넌트가 바로 `PDFTextField`죠. 그러나 기본적으로 렌더링이 표준화되어있지 않기 때문에 뷰어마다 다르게 보이게 되는 것입니다.
