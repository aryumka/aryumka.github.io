...
Unknown은 "개발자가 예측을 못한 실수"가 아니다. 의존성 내부 버그, 특정 데이터 조합에서만 발생하는 edge case, 배포 후 환경 변화로 드러나는 문제 등 **아무리 꼼꼼해도 사전에 모든 실패를 열거하는 것은 불가능하다.** Unknown은 그 열거의 한계를 인정하는 fallback이다.
...
정리하면 **비즈니스 케이스인가 아닌가**가 분류 기준이고 분류에 따라 **처리 메커니즘**이 달라진다.
...
이 에러는 누구의 문제이고 어떻게 대응해야 하는가. 결국 소프트웨어 엔지니어링의 근원적 관심사인 경계와 책임으로 회귀하는 문제다.
